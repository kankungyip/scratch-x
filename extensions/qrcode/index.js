...
class QRCodeBlocks {
    constructor () {
        Scratch.emitter.on('videoSensing.frame', this.setVideoFrame.bind(this));
    ...
    async _encode (content, x, y) {
        if (!(this.attrs.width && this.attrs.height)) {
            const [width, height] = await Scratch.stage.getNativeSize();
...
${this.qrcodeGs.join('\n')}
        ...
        if (!this._drawableId) {
            this._drawableId = await Scratch.stage.createSVGDrawable(Scratch.stage.Layering.SPRITE_LAYER);
        }
        await Scratch.stage.drawSVG(this._drawableId, svg);
    }
